...
def notifyBuild(String buildStatus = 'STARTED', NotifyBitbucket , NotifyHipChat , NotifyEmail , String EmailSubjectStart = 'Build Failed in Jenkins') {
	...
	// Default values
	def colorName = 'RED'
	// def hipchatmessage = "${env.JOB_NAME} ${env.BUILD_NUMBER} ${buildStatus} after ${env.BUILD_DURATION}(${env.HIPCHAT_CHANGES_OR_CAUSE}) (<a href="${env.BUILD_URL}">View build</a>)"
	def hipchatmessage = "${env.JOB_NAME} ${env.BUILD_NUMBER} ${buildStatus} after ${env.BUILD_DURATION}"
