...
//		}
		...
		success {
			...
			notifyBuild('SUCCESSFUL', true, true, true, 'Jenkins Build is Successfull')
		}
//		unstable {
...
def notifyBuild(String buildStatus = 'STARTED', NotifyBitbucket , NotifyHipChat , NotifyEmail , String EmailSubjectStart = 'Build Failed in Jenkins') {
	...
	// Default values
	def colorName = 'RED'
	def hipchatmessage = """${env.JOB_NAME} ${env.BUILD_NUMBER} ${buildStatus} (${env.HIPCHAT_CHANGES_OR_CAUSE}) (<a href="${env.BUILD_URL}">View build</a>)"""
